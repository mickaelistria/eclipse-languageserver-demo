...
export function activate(context: ExtensionContext) {
	...
	languages.setLanguageConfiguration('ski', {
	...
	let serverOptions: Executable = {
		...
		args: [ "-jar", "/home/mistria/workspaceDemoLSP/Le LanguageServer de Chamrousse/target/chamrousse-languageserver-0.0.1-SNAPSHOT-jar-with-dependencies.jar" ]
	};

	// Options to control the language client
	let clientOptions: LanguageClientOptions = {
		// Register the server for ski
		documentSelector: ['ski'],
		synchronize: {
			configurationSection: 'ski',
			// Notify the server about file changes to .java files contain in the workspace
			fileEvents: [
				workspace.createFileSystemWatcher('**/*.ski'),
			],
		}
	};
